...
### Prerequisites
- `ffmpeg-full` must be installed at `/opt/homebrew/opt/ffmpeg-full/bin/ffmpeg` (the standard ffmpeg lacks drawtext/libass support)
...
#### Step 6: Render with ffmpeg-full

```bash
/opt/homebrew/opt/ffmpeg-full/bin/ffmpeg -y -i <input> \
...
**Important**: Must use `ffmpeg-full`, not the standard `ffmpeg`, because drawtext requires libfreetype which is not in the standard Homebrew ffmpeg build.
...
### Important notes
- This skill requires `ffmpeg-full` (installed via `brew install ffmpeg-full`), not the standard `ffmpeg`
